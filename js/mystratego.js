var me={token:null,piece_color:null,alertShown: false};
var game_status={};
var board={};
...
function draw_empty_board(p) {
    if(p!='B') {p='R';}
    var draw_init = {
        'R': {i1:10, i2:0, istep:-1, j1:1, j2:11, jstep:1}, 
        'B': {i1:1, i2:11, istep:1,  j1:10, j2:0, jstep:-1}
    };
    var s = draw_init[p];
...
function fill_board() {
	$.ajax({url: "stratego.php/board/", 
		headers: {"X-Token": me.token},
		success: fill_board_by_data });
}

function reset_board() {
	$.ajax({url: "stratego.php/board/", headers: {"X-Token": me.token}, method: 'POST',  success: fill_board_by_data });
	$('#move_div').hide();
...
function update_status(data) {
	...
	// If the game status has changed to 'ended', make an AJAX call to the PHP script
    if (game_status.status === 'ended' && !me.alertShown) {
		me.alertShown = true;
        $.ajax({
            url: "stratego.php/update_game_status", // The URL where the update_game_status PHP function can be called
            method: 'POST',
            dataType: "json",
            headers: {"X-Token": me.token},

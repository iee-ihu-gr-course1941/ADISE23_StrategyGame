...
function update_game_status() {
	...
	if($status['status'] == 'ended') {
        ...
        $alertMessage = "Game Ended. Winner: " . $winner;;
...
function determine_winner() {
    ...
    $sql = 'SELECT result FROM game_status WHERE status="ended"';
    $st = $mysqli->prepare($sql);
    $st->execute();
    $res = $st->get_result();
    if ($row = $res->fetch_assoc()) {
        // Check the result and return the corresponding color
        if($row['result'] == 'R') {
            return 'Red';
        } elseif($row['result'] == 'B') {
            return 'Blue';
    ...
    return 'No Winner'; // Default return value in case there is no result or if the result is not 'R' or 'B'
}
